...
void Context::add_java_package(const char *package_name)
{
    get_global_name_space()->add_class_identifier(ast::Identifier::create_by_split(package_name, '.'));
}

Identifier *Context::find_identifier(Node *node, const j4a::string& name)
{
    Identifier *qid = NULL;
